 ...
 * @since API 3.0.0
 */
class IntDataProperty extends DataProperty{

	const DATA_TYPE = EntityDataManager::DATA_TYPE_INT;

	/** @var int */
	protected $value;

	public function readFrom(BinaryStream $stream){
		$this->setValue($stream->getVarInt());
	}

	public function writeTo(BinaryStream $stream){
		$stream->putVarInt($this->value);
	}

	public function equals(DataProperty $property) : bool{
		return $property instanceof IntDataProperty and $property->getValue() === $this->value;
	}
	...
	public function setValue($value){
		if(is_int($value)){
			$this->value = (PHP_INT_SIZE === 8 ? ($value << 32 >> 32) : $value);
		}else{
